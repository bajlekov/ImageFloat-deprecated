...
--!!!! FIGURE OUT BLUE CAST NEAR EDGES
...
-- requires FILE declaration if no sdl is loaded, check internally!!
ffi.cdef[[
  struct _IO_FILE;
...
function ppg_filter(p, i, q, j)
  for x = 2, xmax-3 do
    for y = 2, ymax-3 do
      p[x][y][i] = median(q, x, y, j)
    end
  end
end

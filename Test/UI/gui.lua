...
local sdl = require"Source.Include.sdl2" -- investigate higher CPU consumption
sdl.init()
...
function frameFun:getElem(x, y)
	local fr = self:getFrame(x, y)
	if fr.elements and #(fr.elements)>0 then
		if x<fr.x+2 or x>fr.x+fr.w-3 then return end
		y = y + (fr.scroll or 0) - fr.y - 16 -- 14 for header
		local k = fr.elements.map[y]
		if k==nil then return end
		y = y - fr.elements[k].offset
		x = x - fr.x - 2
		print(k, x, y)
		return fr.elements[k], x, y
	end
...
function drawFunctions.elem.float(elem)
	...
	if yp>elemHeight-3 and yp+elemHeight<h-2 then
		local f = (elem.value[1]-elem.value[3])/(elem.value[4]-elem.value[3])
		sdl.draw.color(224, 224, 224)
		sdl.draw.fill(x+2, y+2+yp, w-4, elemHeight-2)
...
function frameFun.drawFrame(frame) -- TODO: make a draw frame draw function
	local f = frame
	local t = 192
...
function frameFun:elem(name, eltype, value)
	...
	if not self.elements then self.elements = {parent = self, map = {}} end
	...
	if data then
		local map = e.map
		local len = e.len or 0
		for i = len, len + elemHeight-3 do
			map[i] = n+1
		end
		...
		e[n+1] = {
			...
			event	= {
				onAction	= pass, -- same implementation as drawing function, pass element, input
				onContext	= pass,
				onHover		= pass,
				onDrag		= pass,
				onWheel		= pass,
...
function frameFun.parseFrames(table)
	-- initial sizes:
	...
	if not fill and totWidth<maxSize then
		...
		for k, v in ipairs(table) do
			...
			v.scroll = 0
			if #v>0 then v:parseFrames() end -- recurse over sub-frames
...
input:elem("Test1")
input:elem("Test2")
input:elem("Test3", "float", {0, 0, -100, 100})
input:elem("Test4")
input:elem("Test5")
view:elem("Test6")
view:elem("Test7")
...
data1.Test3.event.onDrag = onDragFloat
...
-- draw ui
local level = 0
function frameFun.drawElems(table, num)
...
function frameFun.drawFrames(table)
	if #table>0 then
		for k, v in ipairs(table) do
			v:drawFrame()
			if #v>0 then v:drawFrames()
			elseif v.elements then v:drawElems() end
		end
	else
		table:drawFrame()
		if #table>0 then table:drawFrames()
		elseif table.elements then table:drawElems() end
	end
end
...
do
	...
	guiUpdate = function()
		...
		if drag and sdl.input.dx~=0 and fdrag then
